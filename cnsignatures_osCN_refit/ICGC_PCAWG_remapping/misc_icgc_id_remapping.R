...
## All segmentation sample names map but not all segment table samples have activities?
length(unique(icgc_segs$sampleNew))
sum(unique(icgc_segs$sampleNew) %in% rownames(activities))

## Table of project of interest counts are close to the published ones (two extra samples in ESAD-UK which might have had low CIN)
merged_tables_sub <- merged_tables[which(merged_tables$project_code %in% c("OV-AU","ESAD-UK")),]
table(merged_tables_sub$project_code[merged_tables_sub$samplename %in% unique(icgc_segs$sampleNew)])
table(merged_tables_sub$project_code[merged_tables_sub$samplename %in% rownames(activities)])
