...
write.table(x = mapping_alluvial,file = "data/drews2022_V1_V2_signature_mapping.tsv",quote = F,sep = "\t",col.names = TRUE)
```
...
prop_ordering <- c(as.character(threshold_signature_order$signature))

mapping_file_ordered <- mapping_file[match(prop_ordering,mapping_file$name),]
mapping_file_ordered$name_new <- paste0("CN",seq.int(1,nrow(mapping_file_ordered),1))
...
## mapping
Drews2022_original_vs_v2_name_mapping <- mapping_file_ordered
...
Drews2022_v2_CX3CX2_Clinical_classifier <- list(mean = c(CN3 = 0.534446561600389, CN2 = 0.113118812268331),
     scale = c(CN3 = 0.090916422939345, CN2 = 0.0643674353255817))
...
resCoxOS2 = survival::coxph(survival::Surv(OS.time, OS) ~ CN1 + CN2 + CN3 + CN4 + CN5 + CN6 + CN7 + CN8 + CN9 + CN10 + CN11 + CN12 + CN13 + CN14 + CN15 + CN16 + CN17 + CN18 + CN19 + Age + Race + TS, data =  dtOV)
pCoxOS2 = survminer::ggforest(resCoxOS2, data = dtOV, main = "Cox HR model for TCGA-OV OS")

resCoxPFI2 = survival::coxph(survival::Surv(PFI.time, PFI) ~ CN1 + CN2 + CN3 + CN4 + CN5 + CN6 + CN7 + CN8 + CN9 + CN10 + CN11 + CN12 + CN13 + CN14 + CN15 + CN16 + CN17 + CN18 + CN19 + Age + Race + TS, data =  dtOV)
