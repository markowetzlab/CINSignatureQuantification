...
segs_table <- readRDS("0_TCGA_Segments_dCIN.rds")
...
if(!file.exists("osCN_Fix_mixtureModel_newlyfitted.rds")){
    ...
    osCN_Fix_mixtureModel <- readRDS("osCN_Fix_mixtureModel_newlyfitted.rds")
}
```
...
if(!file.exists("osCN_oldrefittedVsnew_mixtures.rds")){
    saveRDS(list(osCN_noFix_mixtureModel,osCN_noFix_mixtureModel_weights,
             osCN_Fix_mixtureModel,osCN_Fix_mixtureModel_weights,
             osCN_original_pub_mixtureModel),file = "osCN_oldrefittedVsnew_mixtures.rds")
}
...
if(!file.exists("Drews2022_v2_TCGA_Mixture_Models.rds")){
    ...
    saveRDS(Drews2022_v2_TCGA_Mixture_Models,file = "Drews2022_v2_TCGA_Mixture_Models.rds")
} else {
    Drews2022_v2_TCGA_Mixture_Models <- readRDS("Drews2022_v2_TCGA_Mixture_Models.rds")
}
...
if(!file.exists("pan_new_oscn_mixture_tcga_SxC.rds")){
    saveRDS(osCN_new_sxc,"pan_new_oscn_mixture_tcga_SxC.rds")
}
if(!file.exists("pan_new_oscn_mixture_tcga_CxS.rds")){
    saveRDS(osCN_new_cxs,"pan_new_oscn_mixture_tcga_CxS.rds")
} 
if(!file.exists("pan_new_oscn_mixture_tcga_CxS.tsv")){
    write.table(osCN_new_cxs,file = "pan_new_oscn_mixture_tcga_CxS.tsv",quote = F,sep = "\t",row.names = T,col.names = T)
}
...
meta.data <- read.table("Metadata_TCGA_ASCAT_penalty70.txt",header = T,sep = "\t")
...
if(!file.exists("nmf_cxs_groups.txt")){
    writeLines(names(splitSxC),con = "nmf_cxs_groups.txt")
}
...
lapply(names(splitSxC),FUN = function(x){
    ...
    grp <- splitSxC[[x]]
    grp_t <- t(grp)
    saveRDS(grp,paste0(groupName,"_new_oscn_mixture_tcga_SxC.rds"))
    saveRDS(grp_t,paste0(groupName,"_new_oscn_mixture_tcga_CxS.rds"))
    write.table(grp_t,file = paste0(groupName,"_new_oscn_mixture_tcga_CxS.tsv"),quote = F,sep = "\t",row.names = T,col.names = T)
})
```
