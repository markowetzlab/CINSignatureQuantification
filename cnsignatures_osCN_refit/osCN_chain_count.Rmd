...
## New 19 linearly independent 

```{r combine_pc_cs_tcga_6335_filtSigs_default19}
# Previous cosine threshold of 0.74 resulted in 19 signatures being retained after PC-CS and CS-CS filtering

filtMatAll19 = removePCSimilarSigs(matAll, COSINETHRESH = 0.74,PREFIXPCSIGS = "pan_cancer")
dim(filtMatAll19)
...
```{r combine_pc_cs_tcga_6335_compSigsDefsall}
...
sigList <- apply(compOldNewCosineFiltThreshAll,MARGIN = 1,FUN = function(x) sort(x[which(x >= 0.74)],decreasing = T))

# If pan_cancer then select pan_cancer else take highest CS
...
## Direct intervention for CX15 and corresponding new sig as pan_cancer_W3 appeared twice
...
Heatmap(compOldNewCosineFiltThreshAll_sub, cluster_columns = F, cluster_rows = FALSE,
        column_title = "Signature definition comparison (cosine >= 0.74)",name = "cosine sim",
        cell_fun = function(j, i, x, y, w, h, col) {
            if(round(compOldNewCosineFiltThreshAll_sub,digits = 2)[i, j] > 0){# add text to each grid
        grid.text(round(compOldNewCosineFiltThreshAll_sub,digits = 2)[i, j], x, y)
            }
    })

pcvcsCorr <- corMat[rownames(corMat) %in% selectedList$new,colnames(corMat) %in% selectedList$new]
Heatmap(pcvcsCorr,column_title = "PC vs CS linear dependence",name = "cosine sim",
        cluster_columns = F, cluster_rows = FALSE,
        cell_fun = function(j, i, x, y, w, h, col) {
            if(round(pcvcsCorr,digits = 2)[i, j] > 0.74){# add text to each grid
        grid.text(round(pcvcsCorr,digits = 2)[i, j], x, y)
            }
    })
...
Heatmap(oldSigDefs, cluster_columns = FALSE, cluster_rows = FALSE, column_title = "Drews 2022 signatures",name="weight")
Heatmap(matAll[rownames(matAll) %in% colnames(compOldNewCosineFiltThreshAll_sub),],row_order = colnames(compOldNewCosineFiltThreshAll_sub), cluster_columns = FALSE, cluster_rows = FALSE, column_title = "New pan-cancer signature compendium",name = "weight")
```

```{r get_sig_acts_new_definitions}
osCN_new_sxc <- readRDS("data/pan_new_oscn_mixture_tcga_SxC.rds")
sigDef <- matAll[rownames(matAll) %in% colnames(compOldNewCosineFiltThreshAll_sub),]
newActs_NewSig_NewMix <- CINSignatureQuantification:::calculateActivityDrewsV2(object = osCN_new_sxc,
                                                      SIGS = matAll[rownames(matAll) %in% colnames(compOldNewCosineFiltThreshAll_sub),])

lSigs <- t(apply(newActs_NewSig_NewMix,2,function(x) x/sum(x)))
head(t(newActs_NewSig_NewMix))
```
