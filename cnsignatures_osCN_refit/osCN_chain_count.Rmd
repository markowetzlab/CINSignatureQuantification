...
PANCANCERSIGS="nmfruns/solutions/pan_cancer/2_Signatures_ENBKKW_normalised.rds"
PANCANCEREXP="nmfruns/solutions/pan_cancer/2_Exposures_ENBKKW_normalised.rds"
...
ggplot(dtExposedSamples, aes(x = Samples, fill = Cancer)) + geom_histogram() + scale_x_log10() + 
...
```{r combine_pc_cs_tcga_6335_filtSigs}
print(paste0("previous cosine threshold - ",COSINETHRESH))
...
filtMatAll = removePCSimilarSigs(matAll, COSINETHRESH = 0.72,PREFIXPCSIGS = "pan_cancer")
dim(filtMatAll)

## Step 3: CS sigs vs CS sig (retain signature with most exposed samples)
csSigs = removeCSSimilarSigs(filtMatAll, dtExposedSamples, COSINETHRESH = 0.72)
dim(csSigs)
...
```{r combine_pc_cs_tcga_6335_combinedSigs,message=FALSE}
Heatmap(matPCSC, cluster_columns = FALSE, cluster_rows = FALSE, column_title = "New pan-cancer signature compendium",name = "weight")

mOther = matPCSC
mOther = apply(mOther, 2, function(y) y/sum(y))
#Heatmap(mOther, cluster_columns = FALSE, column_title = "PCSC (normalised per component)",name = "weight")
...
```{r combine_pc_cs_tcga_6335_oldSigs,message=FALSE}
...
```{r combine_pc_cs_tcga_6335_compSigsDefs}
compOldNewCosine <- cosine(t(rbind(CINSignatureQuantification::Drews2022_TCGA_Signatures,matPCSC)))
compOldNewCosineFilt <- compOldNewCosine[,!colnames(compOldNewCosine) %in% paste0("CX",seq_len(17))]
compOldNewCosineFilt <- compOldNewCosineFilt[rownames(compOldNewCosineFilt) %in% paste0("CX",seq_len(17)),]

compOldNewCosineFiltThresh <- compOldNewCosineFilt
Heatmap(compOldNewCosineFiltThresh, cluster_columns = F, cluster_rows = FALSE,
        column_title = "Signature definition comparison (no threshold)",name = "cosine sim")

compOldNewCosineFiltThresh[compOldNewCosineFiltThresh < 0.72] = 0
sum(apply(compOldNewCosineFiltThresh,MARGIN = 2,FUN = function(x) sum(any(x >= 0.72))))
Heatmap(compOldNewCosineFiltThresh, cluster_columns = F, cluster_rows = FALSE,
        column_title = "Signature definition comparison (cosine >= 0.72)",name = "cosine sim")

compOldNewCosineFiltThresh[compOldNewCosineFiltThresh < 0.90] = 0
sum(apply(compOldNewCosineFiltThresh,MARGIN = 2,FUN = function(x) sum(any(x >= 0.90))))
Heatmap(compOldNewCosineFiltThresh, cluster_columns = F, cluster_rows = FALSE,
        column_title = "Signature definition comparison (cosine >= 0.90)",name = "cosine sim")

compOldNewCosineFiltThresh[compOldNewCosineFiltThresh < 0.95] = 0
sum(apply(compOldNewCosineFiltThresh,MARGIN = 2,FUN = function(x) sum(any(x >= 0.95))))
Heatmap(compOldNewCosineFiltThresh, cluster_columns = F, cluster_rows = FALSE,
        column_title = "Signature definition comparison (cosine >= 0.95)",name = "cosine sim")
