...
```{r calc_newfixed_osCN_tcga_6335_withPos_label}
osCN_fix_labelled <- getOscillationDrewsV2WITHLABEL(segs)
dim(osCN_fix_labelled)
```

```{r newfixed_osCN_tcga_6335_withPos_label_dists}
osCN_fix_labelled$value <- as.numeric(osCN_fix_labelled$value)

## Zero counts only possible where label == FALSE so removed
osCN_fix_labelled_noZero <- osCN_fix_labelled[!c(osCN_fix_labelled$value == 0 & osCN_fix_labelled$label == FALSE),]

## Summary distributions for each label
summary(osCN_fix_labelled_noZero$value[osCN_fix_labelled_noZero$label == FALSE])
summary(osCN_fix_labelled_noZero$value[osCN_fix_labelled_noZero$label == TRUE])

## Chain length quantiles > 0.99
quantile(osCN_fix_labelled_noZero$value,probs = seq.int(0.99,1,0.0005))

## percentage of chains > length 30
sum(osCN_fix_labelled_noZero$value > 30) / length(osCN_fix_labelled_noZero$value) * 100

ggplot(osCN_fix_labelled_noZero[osCN_fix_labelled_noZero$value < 33,]) +
    geom_density(aes(value,fill=label,after_stat(ndensity)),adjust=5,alpha=0.3,position = "stack") +
    #facet_wrap(. ~ label) +
    theme_bw()

## poisson test
poisson_counts_labelled <- sum(osCN_fix_labelled_noZero$value[osCN_fix_labelled_noZero$label == TRUE])
poisson_events_labelled <- length(osCN_fix_labelled_noZero$value[osCN_fix_labelled_noZero$label == TRUE])
poisson_counts <- sum(osCN_fix_labelled_noZero$value[osCN_fix_labelled_noZero$label == FALSE])
poisson_events <- length(osCN_fix_labelled_noZero$value[osCN_fix_labelled_noZero$label == FALSE])
poisson.test(c(poisson_counts_labelled,poisson_counts),T = c(poisson_events_labelled,poisson_events))
```
...
write.table(x = mapping_alluvial,file = "data/drews2022_V1_V2_signature_mapping.tsv",quote = F,sep = "\t",row.names = F,col.names = TRUE)
```
...
threshold_signature_order <- data.frame(prop_act=sort(apply(ThreshNormaliseNewActs_NewSig_NewMix,MARGIN = 2,
                                                            FUN = function(x) sum(x > 0) / length(x)),decreasing = T)) %>%
...
plotCosineHeatmap(t(tcga_gold_orig_filt),t(tcga_gold_new_filt[,as.character(threshold_signature_order$signature[threshold_signature_order$signature %in% newNames])]),threshold = 0.4,title = "Signature activity (TCGA gold) - mapped")
```

It is also worth comparing the rate of zero-activities between the two implementations to identify if mapped signatures have differing rates of zeros.
...
```{r rename_and_map}
mapping_file <- read.table("data/drews2022_V1_V2_signature_mapping.tsv",header = T,sep = "\t")
prop_ordering <- c(as.character(threshold_signature_order$signature),"old unmapped")

mapping_file_ordered <- mapping_file[match(prop_ordering,mapping_file$name),]
mapping_file_ordered$name_new <- c(paste0("CS",seq.int(1,nrow(mapping_file_ordered)-1,1)),"old unmapped")
mapping_file_ordered <- mapping_file_ordered[,c("name_new","name","original","cosineSim")]
colnames(mapping_file_ordered) <- c("drews2022_V2","name","drews2022_original","cosineSim")
mapping_file_ordered
```

```{r orderedSigDefsandActs}
## mapping
Drews2022_original_vs_v2_name_mapping <- mapping_file_ordered[mapping_file_ordered$name != "old unmapped",]

## Thresholds file
Drews2022_v2_TCGA_Signature_Thresholds <- Drews2022_V2_TCGA_Signature_Thresholds[mapping_file_ordered$name]
names(Drews2022_v2_TCGA_Signature_Thresholds) <- mapping_file_ordered$drews2022_V2
Drews2022_v2_TCGA_Signature_Thresholds

## Signature defs
Drews2022_v2_TCGA_Signatures <- osCN_new_sigDef[order(match(rownames(osCN_new_sigDef),prop_ordering)),]
rmlst <- length(mapping_file_ordered$drews2022_V2[match(prop_ordering,mapping_file_ordered$name)])
rownames(Drews2022_v2_TCGA_Signatures) <- mapping_file_ordered$drews2022_V2[match(prop_ordering,mapping_file_ordered$name)][-rmlst]
head(Drews2022_v2_TCGA_Signatures)

## Signature act scaling
sigActorderingVec <- order(match(colnames(ThreshNormaliseNewActs_NewSig_NewMix),prop_ordering))
ThreshNormaliseNewActs_NewSig_NewMix_ordered <- ThreshNormaliseNewActs_NewSig_NewMix[,sigActorderingVec]
colnames(ThreshNormaliseNewActs_NewSig_NewMix_ordered) <- mapping_file_ordered$drews2022_V2[match(prop_ordering,mapping_file_ordered$name)][-rmlst]
head(ThreshNormaliseNewActs_NewSig_NewMix_ordered) # Drews2022_TCGA_v2_SignatureActs (not exported)

## scaling variables
ThreshNormaliseNewActs_NewSig_NewMix_ordered_scaled <- scale(as.matrix(ThreshNormaliseNewActs_NewSig_NewMix_ordered))
Drews2022_v2_TCGA_Scaling_Variables <- list(mean=attributes(ThreshNormaliseNewActs_NewSig_NewMix_ordered_scaled)$`scaled:center`,
                                            scale=attributes(ThreshNormaliseNewActs_NewSig_NewMix_ordered_scaled)$`scaled:scale`)
Drews2022_v2_TCGA_Scaling_Variables

## CN2CN3 scaling parameters
## This is computed downstream of this point in the clinical classifier section 
## But saved as input string to allow export here.
Drews2022_v2_CX3CX2_Clinical_classifier <- list(mean = c(CS3 = 0.534446561600389, CS2 = 0.113118812268331),
     scale = c(CS3 = 0.090916422939345,CS2 = 0.0643674353255817))
Drews2022_v2_CX3CX2_Clinical_classifier
```

```{r mapping_aluvial_2}
ggplot(mapping_file_ordered,
       aes(axis1=factor(drews2022_V2,levels=stringr::str_sort(drews2022_V2,numeric = T)),
           axis2=factor(drews2022_original,levels=stringr::str_sort(unique(drews2022_original),numeric = T)))) +
    geom_alluvium(aes(fill=as.numeric(cosineSim))) +
    scale_fill_gradient2(low="white", high="red",name = "cosine similarity",limits = c(0,1)) +
    geom_stratum(width = 0.4, fill = "white") +
    geom_text(stat = "stratum",size = 5, aes(label = after_stat(stratum))) +
    theme_void() +
    theme(legend.position = "bottom",
...
```{r requiredDataForV2}
d <- as.data.frame(data(package="CINSignatureQuantification")$results)
d[grep("Drews",d$Item),]
```
...
## Clinical classifier 
Drews2022_v2_CX3CX2_Clinical_classifier
# usethis::use_data(Drews2022_v2_CX3CX2_Clinical_classifier)

## Mixture models
Drews2022_v2_TCGA_Mixture_Models
# usethis::use_data(Drews2022_v2_TCGA_Mixture_Models)

## Scaling variables
Drews2022_v2_TCGA_Scaling_Variables
# usethis::use_data(Drews2022_v2_TCGA_Scaling_Variables)

## Signature thresholds
Drews2022_v2_TCGA_Signature_Thresholds
# usethis::use_data(Drews2022_v2_TCGA_Signature_Thresholds)

## signature defs
Drews2022_v2_TCGA_Signatures
# usethis::use_data(Drews2022_v2_TCGA_Signatures)

## Mapping file data
Drews2022_original_vs_v2_name_mapping
# Write this file to data/
# usethis::use_data(Drews2022_original_vs_v2_name_mapping)
...
```{r gene_interactions_load_data}
## Load files
#raw = readRDS(RAW)
newActs_NewSig_NewMix_raw <- t(calculateActivityDrewsV2(object = osCN_new_sxc,SIGS = osCN_new_sigDef))[,sigActorderingVec]
colnames(newActs_NewSig_NewMix_raw) <- Drews2022_original_vs_v2_name_mapping$drews2022_V2
raw <- newActs_NewSig_NewMix_raw
#meta = readRDS(META)
meta.data = meta.data

# Driver
cns = readRDS("../../Drews2022_CIN_Compendium/Section 7 Identification of putative signature aetiologies/Section 7.1 Linking signature activities to mutated genes/input/CN_State_GoIs_TCGA.rds")

dels = cns[ cns$consequence == "DEL", ]
amps = cns[ cns$consequence == "AMP", ]

snv = readRDS("../../Drews2022_CIN_Compendium/Section 7 Identification of putative signature aetiologies/Section 7.1 Linking signature activities to mutated genes/input/SNV_Status_GoIs_TCGA.rds")

# For providing additional data on BRCA1/2, RAD51C mutated samples
brcaMuts = readRDS("C:/Users/smith10/Git_repo/Drews2022_CIN_Compendium/Section 7 Identification of putative signature aetiologies/Section 7.1 Linking signature activities to mutated genes/input/TCGA_Exposures_and_BRCA_Status_plusGene.rds")

brca1 = unique(as.character(brcaMuts$Sample[ grepl("[germline|somatic] BRCA1",brcaMuts$Status) ]))
brca2 = unique(as.character(brcaMuts$Sample[ grepl("[germline|somatic] BRCA2",brcaMuts$Status) ]))
...
dtGeneLookup = fread(file = "C:/Users/smith10/Git_repo/Drews2022_CIN_Compendium/Section 7 Identification of putative signature aetiologies/Section 7.1 Linking signature activities to mutated genes/input/Reactome_and_Driver_genes_and_their_IDs.txt")
...
oncogenes = c(oncogenes, "CCNE1")

# Ruben selected all except FS but should realistically not select nonsense 
# or splice site as these are more likely LoF in the context of an Oncogene
...
tsgs = tsgs[ tsgs != "CCNE1" ]

## Again, ruben did not filter on consequence, leaving ncRNA variants included
...
```{r gene_to_signatures_univariate}
...
dtSigMuts$Cancer = meta.data$cancer_type[ match(dtSigMuts$Samples, meta.data$name) ]
...
```{r gene_to_signatures_multivariate}
...
GENEDETAILS <- dtGeneLookup
REACTOMEDETAILS=fread(file = "C:/Users/smith10/Git_repo/Drews2022_CIN_Compendium/Section 7 Identification of putative signature aetiologies/Section 7.1 Linking signature activities to mutated genes/input/Reactome_genes_pathway_dictionary.txt")
...
# dtGenes = fread(file = "../Drews2022_CIN_Compendium/Section 7 Identification of putative signature aetiologies/Section 7.1 Linking signature activities to mutated genes/input/Reactome_and_Driver_genes_and_their_IDs.txt")
## Convert labels in lookup data frame
dtGenes$Status[ grepl("oncogene", dtGenes$Status) ] = "Oncogene"
dtGenes$Status[ grepl("tsg", dtGenes$Status) ] = "TSG"
dtGenes$Status[ dtGenes$Status == "Ambivalent" ] = "Either"

dtReact <- REACTOMEDETAILS
dtGoI <- TGOI
...
dtSigGoI$Signature = factor(dtSigGoI$Signature, levels = rev(paste0("CS", 1:19)))
...
pC3 = ggplot(dtPlotC3[!is.na(dtPlotC3$Signature),], aes(y = Signature, x = Driver, colour = MeanDiff, shape = SampleCat)) +
  geom_blank() +
  annotate("rect",
           xmin = (seq(1, numGenes, by = 2) - 0.5),
           xmax = (seq(1, numGenes, by = 2) + 0.5),
...
## Load data
exp=ThreshNormaliseNewActs_NewSig_NewMix_ordered
meta=meta.data
...
## HRD Clinical classifier and coxPH models

```{r coxPH_CX2CX3-CN2CN3}
## Load data from Rmd MultiFigures_Clinical_classifier
dtSurvOV = readRDS("../../Drews2022_CIN_Compendium/Section 9 Building a clin classifier/input/Survival_and_BRCA_Status_TCGA-OV.rds")
# Drews2022_CIN_Compendium/Section 9 Building a clin classifier/input/Survival_and_BRCA_Status_TCGA-OV.rds

metaFull = meta.data

mTCGA = ThreshNormaliseNewActs_NewSig_NewMix_ordered
mLRTCGA = apply(mTCGA, 2, function(x) compositions::clr(x))
...
resCoxOS2 = survival::coxph(survival::Surv(OS.time, OS) ~ CS1 + CS2 + CS3 + CS4 + CS5 + CS6 + CS7 + CS8 + CS9 + CS10 + CS11 + CS12 + CS13 + CS14 + CS15 + CS16 + CS17 + CS18 + CS19 + Age + Race + TS, data =  dtOV)
pCoxOS2 = survminer::ggforest(resCoxOS2, data = dtOV, main = "Cox HR model for TCGA-OV OS")

resCoxPFI2 = survival::coxph(survival::Surv(PFI.time, PFI) ~ CS1 + CS2 + CS3 + CS4 + CS5 + CS6 + CS7 + CS8 + CS9 + CS10 + CS11 + CS12 + CS13 + CS14 + CS15 + CS16 + CS17 + CS18 + CS19 + Age + Race + TS, data =  dtOV)
pCoxPFI2 = survminer::ggforest(resCoxPFI2, data = dtOV, main = "Cox HR model for TCGA-OV PFI")
...
```{r CX2CX3-CN2CN3_classifier}
applyClinClassCN = function(dtSurvOV, lModel) {
  
  ## Scale the activities
  mTCGAOV = as.matrix(dtSurvOV[ , c("CN3", "CN2")])
  rownames(mTCGAOV) = dtSurvOV$Name
  smTCGAOV = sweep(sweep(mTCGAOV, 2, lModel$mean, FUN = '-'), 2, lModel$scale, FUN = "/")
  
  dtSurvOV$sCN3 = smTCGAOV[,"CN3"]
  dtSurvOV$sCN2 = smTCGAOV[,"CN2"]
  
  ## Apply classifier
  if(identical(dtSurvOV$Name, rownames(smTCGAOV))) {
    dtSurvOV$ClassifierCN = ifelse(dtSurvOV$sCN3 >= dtSurvOV$sCN2, "Predicted sensitive", "Predicted resistant")  
  } else {
...
mTCGAdf <- as.data.frame(mTCGA[match(dtSurvOV$Name,rownames(mTCGA)),])

all(rownames(mTCGAdf) == dtSurvOV$Name)

dtSurvOV$CN3 <- mTCGAdf$CN3
dtSurvOV$CN2 <- mTCGAdf$CN2

mBRCA1 = as.matrix(dtSurvOV[ dtSurvOV$Status == "germline BRCA1+LOH", c("CN3", "CN2")])
...
dtSurvOV = applyClinClassCN(dtSurvOV, lModel)

## Plot for figure
pOut = ggplot(dtSurvOV[ dtSurvOV$Status == "germline BRCA1+LOH", ], aes(x = sCN3, y = sCN2, colour = ClassifierCN)) + 
  geom_point() + geom_abline(slope = 1, intercept = 0, colour = "grey30", linetype = "dashed") +
  theme(legend.position = "none", axis.line = element_line(linewidth = 0.5), 
        axis.ticks = element_line(size = 0.5), axis.ticks.length = unit(.1, "cm"), 
        plot.margin = unit(c(0, 0, 0, 0), "null"),) + 
  lemon::coord_capped_cart(bottom = "both", left = "both", ylim = c(-2, 2)) +
  labs(x = "Scaled CN3 activity", y = "Scaled CN2 activity") + theme(aspect.ratio = 1)
pOut
```
